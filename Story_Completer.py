...
def get_valid_event_mask(data_entry):
    ...
    return output, valid_eventIDs, user_input, action
    #print(f"Predicted text to be modified/removed: {output}")
    #print(f"Original text: {target_text}")

# MAIN FUNCTION: replace the generated mask span in the original story
def regenerate_story(storychunks, valid_eventIDs, pred_ID, pred_output, action):
    result = ""
    for i in range(len(storychunks)):
        curr_chunk = storychunks[i]
        curr_ID = valid_eventIDs[i]
        if curr_ID == pred_ID:
            if action == 'modify':
                result += f"|| MODIFIED START || {pred_output} || MODIFIED END || "
            elif action == 'delete':
                result += f"|| ......THIS EVENT REMOVED...... ||"
        else:
            result += curr_chunk
    return result
...
def run_interactive_main_loop():
    ...
    while True:
        ...
        # script summarising loop
            ...
            print("------------------------------------------------------------------------\n") 
            json_outline, storychunks = generate_json_outline(story,
            ...
            print("------------------------------------------------------------------------\n") 
            # select the event id to edit, then generate prediction
            pred_event, valid_eventIDs, maskedID, action = get_valid_event_mask(dataset[INDEX])
            print("------------------------------------------------------------------------\n")
            # regenerate story
            newstory = regenerate_story(storychunks, valid_eventIDs, maskedID, pred_event, action)
